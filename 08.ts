;(() => {
  function loadArray(): { operation: string, value: number, isVisited: boolean }[] {
    return document
    .querySelector('pre')
    .innerText.trim()
    .split('\n')
  ...
  function runProgram(instructions: { operation: string, value: number, isVisited: boolean }[]) {
    let v = 0
    let i: number
  ...
  console.log('swapped the', swappingI, 'th instruction. Now the value is', value)
